...
# to the `dynld` meta-loader, which will search the program's RPATH for a libc loader named ld-linux-x86-64.so.2.
...
let
  ...
  loader = (pkgs.callPackage ../pkgs/dynld.nix { }).loader;
...
in {
  options.dynld = {
    enable = lib.mkOption {
      ...
      default = true;
      description = ''
        This option enables the dynld meta-loader by installing a symlink from the FHS ld loader location to dynld.
      '';
    };
  };
